...
var YoukuUploader = function () {
    ...
    _createClass(YoukuUploader, [{
        ...
        value: function get_upload_token(callback) {
            ...
            _cuddle2.default.get.to(this.UPLOAD_TOKEN_URL).send(this.metadata).end(this.save_upload_token.bind(this, callback));
        }
    }, {
        ...
        value: function create_file(callback) {
            ...
            if (!this.base_url) {
                ...
                return _dns2.default.resolve4(this.token_info.upload_server_uri, function (err, result) {
                    ...
                    _this.upload_ip = result[Math.floor(Math.random() * result.length)];
                    _this.base_url = 'http://' + _this.upload_ip + '/gupload/';
            ...
            _cuddle2.default.post.to(this.base_url + 'create_file').send(params).end(callback);
        }
    }, {
        key: 'upload',
        ...
        value: function new_slice(callback) {
            ...
            _cuddle2.default.get.to(this.base_url + 'new_slice').send({ upload_token: this.token_info.upload_token }).end(function (err, result, request) {
                if (err) {
                    return callback(err);
                }
        ...
        value: function check(callback) {
            ...
            _cuddle2.default.get.to(this.base_url + 'check').send(params).end(function (err, result) {
                if (err) {
                    return callback(err);
                }
        ...
        value: function commit(callback) {
            ...
            _cuddle2.default.get.to(this.UPLOAD_COMMIT_URL).send(params).end(callback);
        }
    }, {
        key: 'on_video_id',
